...
| `GITHUB_APP_ID` / `GITHUB_APP_SLUG` / `GITHUB_APP_PRIVATE_KEY` / `GITHUB_APP_WEBHOOK_SECRET` | Optional — enables the GitHub App ("Connect with GitHub") auth mode alongside the existing personal-access-token mode. Empty `GITHUB_APP_ID` (default) means unconfigured; the PAT mode needs none of these. **Fallback only**: a `GitHubAppConfig` DB row (produced by the in-app "Set up GitHub App" manifest flow) takes precedence over these when one exists. See `docs/integrations/github.md`'s "GitHub App auth mode" and `docs/adr/ADR-004-github-app-auth.md` decision #9. |
